...
@click.command()
@click.option('-f', '--format', type=click.Choice(['cwl', 'galaxy']),
              help="""Input file format (Galaxy Format 2 or Common Workflow Language).""")
@click.option(
        "-i", "--input", type=click.File(),
        help="""Path to input file.""")
@click.option(
        "-o", "--output", type=str,
        help="""Output file name.""", required=False)
def main(format, input, output):
    """Script that converts CWL and Galaxy to ConceptMaps"""
    if format == 'galaxy':
        galaxyWorkflow = json.load(input)
        format2fromGalaxy = gxformat2.from_galaxy_native(galaxyWorkflow[0], json_wrapper=True)
    ...
    if output:
        elements.toJSON(output)
    else:
        print(elements.toJSON())
